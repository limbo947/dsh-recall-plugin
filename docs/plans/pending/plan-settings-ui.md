...
4. 功能回归：配置改-存-回读 ✅（50→55 保存成功 + 4s 消息消退 + 回读持久化）、排除编辑保存 ✅（未修改禁用/修改可存）、快照树搜索 / 三级删除 / 全部删除 / gc 结构 ✅（真实 9 快照数据，删除路径经确认条取消验证未误删；gc 执行按钮在位）、健康行 ✅（git 可用绿 pill）、错误区条件渲染 ✅（当前无错误不显示）。
