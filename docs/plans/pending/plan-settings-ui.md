...
> 状态：自动化冒烟已执行（2026-09-04，执行记录见 [smoke-checklist-records.md](../completed/smoke-checklist-records.md) 第八节）——V1–V9 全部核验通过（link 模式 + dsh 0.1.2-rc.1 实弹，浅/深双主题对照 + 功能回归 + 控制台零报错）。剩余 3 项人工复核（真实窄视口终验 / 真实键盘 Enter·Space / 读屏播报）不阻塞代码质量，随发版前人工冒烟补齐；补齐前计划状态保持「已实施（待冒烟）」留 pending/。

1. 键盘走查：Tab 可达与焦点环 ✅（自动化验证）；树折叠 Enter/Space 真实键触发 △（自动化合成事件无 isTrusted，待人工）；状态消息读屏播报 △（待人工，Narrator/NVDA）。
2. 视觉对照：浅色 / 深色主题各过一遍设置页三卡片 + 撤回确认面板 ✅（2026-09-04 双主题实弹；撤回确认面板的 badge 属 V1 改色、通用样式沿用 V7 收敛，随将至撤回批次冒烟复查）。
3. 窄屏：360–480px 宽面板走查表单与排除编辑器 △（480px 断点已注入等价样式模拟验证：表单单列、快速添加独占一行、无横向溢出；真实窄视口终验待人工）。
4. 功能回归：配置改-存-回读 ✅（50→55 保存成功 + 4s 消息消退 + 回读持久化）、排除编辑保存 ✅（未修改禁用/修改可存）、快照树搜索 / 三级删除 / 全部删除 / gc 结构 ✅（真实 9 快照数据，删除路径经确认条取消验证未误删；gc 执行按钮在位）、健康行 ✅（git 可用绿 pill）、错误区条件渲染 ✅（当前无错误不显示）。**断 git 失败态（V6）** △：PATH 移除模拟方法本机不可行（resolveGitScript 硬编码标准路径探测 + Windows ProgramFiles 子进程不可继承篡改，详见 records 第八节附），失败态改代码级镜像核验（同一渲染路径条件取反），待隔离环境（CI 容器不装 git）终验。
5. 系统减少动效设置下过渡自动关闭 △（`prefers-reduced-motion: reduce` 媒体查询已注入，系统级动效开关实弹随人工冒烟补）。
